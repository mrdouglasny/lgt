...
The U(n) specialization in \texttt{ym\_mass\_gap\_exponential\_decay}
supplies:
\begin{itemize}
...
\S16.3 Application (framework) &
  \texttt{ym\_mass\_gap\_strong\_coupling} \\
Theorem 16.3.1 (exponential decay) &
  \texttt{ym\_mass\_gap\_exponential\_decay}
  \emph{(target; 1 sorry)} \\
\bottomrule
\end{tabular}
\end{table}
...
Sorry count (lgt, \texttt{ym\_mass\_gap\_strong\_coupling}) & 0 \\
Sorry count (lgt, \texttt{ym\_mass\_gap\_exponential\_decay}) & 1 \\
